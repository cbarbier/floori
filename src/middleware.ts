...
export default async function (request: NextRequest) {
	...
	const isPrefixed = !!T.find((t) =>
		t.translations?.filter((x) => x)?.some(({ slug }) => slug === pathname),
	)
	...
	const available = T?.find((t) =>
		[
			t.slug,
			...(t.translations?.filter((x) => x)?.map(({ slug }) => slug) ?? []),
		].includes(pathname),
	)
