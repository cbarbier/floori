...
export default function TileLarge({
	...
	const width = half !== '0' ? counters.w / 2 : counters.w
	const height = counters.h * duration + (duration > 1 ? duration * 1 : 0)
	const delta = half === '2' ? 79 : 0

	const [h, min] = hour?.split(':').map((x) => +x)
	const x = h - 9
	const baseTop = 32 + 80 * x + (min / 60) * 80
	const top = baseTop + (h > 13 ? 2 : 0)
	const left = delta + 248 + counters.w * (day - 1)
	return (
		<div
		...
		>
			...
			<div
				className={cn('title font-inter text-[0.7rem] font-[600]', {
					'break-all': breakword,
				})}
			>
				{text}
